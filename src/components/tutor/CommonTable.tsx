...
interface DataTableProps<T> {
    ...
    // Pass all three to enable server-side / API-driven pagination.
    // Omit all three to use built-in client-side pagination.
    currentPage?: number;        // current active page (1-based)
    totalPages?: number;         // total pages from server
    totalCount?: number;         // total record count from server (for "Showing X–Y of Z")
...
const CommonTable = <T extends Record<string, any>>({
    ...
    // Is the parent driving pagination?
    const isControlled = controlledPage !== undefined && controlledTotalPages !== undefined && onPageChange !== undefined;
    ...
    // ── Client-side filter (only in uncontrolled mode — server filters in controlled mode) ──
    const filteredData = useMemo(() => {
        if (isControlled) return data; // server already filtered
        return data.filter((row) =>
    ...
    }, [data, activeFilters, isControlled]);
    ...
    // ── Pagination ────────────────────────────────────────────────────────────
    // Controlled: data is already the current page's slice → render as-is
    // Uncontrolled: slice locally
    const displayData = useMemo(() => {
        if (isControlled) return sortedData;
        const start = (currentPage - 1) * pageSize;
        return sortedData.slice(start, start + pageSize);
    }, [isControlled, sortedData, currentPage, pageSize]);
    ...
    // "Showing X – Y of Z"
    const totalEntries = isControlled
        ? (controlledTotalCount ?? controlledTotalPages! * pageSize)
        : sortedData.length;
    const showingFrom = totalEntries === 0 ? 0 : (currentPage - 1) * pageSize + 1;
    const showingTo   = isControlled
        ? showingFrom - 1 + (data.length)          // actual rows in this page
    ...
    const handleApplyFilter = () => {
        if (selectedFilterColumn && filterInputValue.trim()) {
            ...
            setActiveFilters([
                ...activeFilters.filter((f) => f.column !== selectedFilterColumn),
                newFilter,
            ]);
            setSelectedFilterColumn('');
            setFilterInputValue('');
            ...
            <div className="p-4 border-b border-gray-200">
                ...
                <div className="flex justify-between items-center mb-3">
                    ...
                    <div className="flex gap-2">
                        ...
                        <div className="relative">
                            ...
                            {showFilterModal && (
                                <>
                                    <div className="fixed inset-0 z-10" onClick={handleCancelFilter} />
                                    <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-xl border border-gray-200 z-20">
                                        <div className="px-4 py-3 border-b border-gray-200">
                                            <h3 className="font-semibold text-sm text-gray-700">Add Filter</h3>
                                        </div>
                                        <div className="p-4 space-y-4">
                                            <div>
                                                ...
                                                <select
                                                    value={selectedFilterColumn}
                                                    onChange={(e) => { setSelectedFilterColumn(e.target.value); setFilterInputValue(''); }}
                                                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                                                >
                                                    <option value="">Choose a column...</option>
                                                    {filterableColumns.map((col) => (
                                            ...
                                            {selectedFilterColumn && (
                                                <div>
                                                    ...
                                                    <input
                                                        type={selectedColumn?.filterType === 'date' ? 'date' : selectedColumn?.filterType === 'number' ? 'number' : 'text'}
                                                        value={filterInputValue}
                                                        onChange={(e) => setFilterInputValue(e.target.value)}
                                                        placeholder={selectedColumn?.filterType === 'number' ? 'Enter number...' : 'Enter text...'}
                                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                                                    />
                                                </div>
                                            )}
                                        ...
                                        <div className="px-4 py-3 border-t border-gray-200 flex justify-end gap-2">
                                            <button onClick={handleCancelFilter} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">
                                                Cancel
                                            </button>
                                            <button
                                                onClick={handleApplyFilter}
                            ...
                            {showColumnMenu && (
                                <>
                                    ...
                                    <div className="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-xl border border-gray-200 z-20 py-2">
                                        ...
                                        {columns.map((col) => (
                                            <label key={col.key as string} className="flex items-center px-4 py-2 hover:bg-gray-50 cursor-pointer">
                                                <input
                                                    type="checkbox"
                                                    checked={visibleColumns[col.key as string]}
                                                    onChange={() => toggleColumn(col.key as string)}
                ...
                {activeFilters.length > 0 && (
                    <div className="flex flex-wrap gap-2 items-center">
                        <span className="text-sm font-medium text-gray-600">Active Filters:</span>
                        {activeFilters.map((filter) => (
                            <div key={filter.column} className="flex items-center gap-2 px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm">
                                <span className="font-medium">{filter.label}:</span>
                                <span>{filter.value}</span>
                                <button onClick={() => removeFilter(filter.column)} className="hover:bg-blue-200 rounded-full p-0.5 transition-colors">
                                    <X size={14} />
                                </button>
                            </div>
                        ))}
                        <button onClick={clearAllFilters} className="text-sm text-red-600 hover:text-red-700 font-medium underline">
                            Clear All
                        </button>
                    </div>
                )}
                            ...
                            {visibleColumnsArray.map((col) => (
                                <th key={col.key as string} className="px-4 py-3 text-left">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {col.sortable && (
                                            <button onClick={() => handleSort(col.key as string)} className="p-1 hover:bg-gray-200 rounded transition-colors">
                                                {sortConfig.key === col.key ? (
                                                    sortConfig.direction === 'asc'
                                                        ? <ChevronUp size={14} className="text-blue-600" />
                                                        : <ChevronDown size={14} className="text-blue-600" />
                        ...
                        {displayData.map((row) => (
                            <tr key={row[rowKey as string]} className="hover:bg-gray-50 transition-colors">
                                {visibleColumnsArray.map((col) => {
                                    const value = row[col.key as string];
                                    const displayValue = col.render ? col.render(value, row) : value;
                                    const cellClass = col.cellClassName ? col.cellClassName(value, row) : '';
                                    return (
                                        <td
                                            key={col.key as string}
                                            className={`px-4 py-3 text-sm text-gray-700 ${cellClass}`}
                ...
                <div className="p-4 border-t border-gray-200 flex items-center justify-between flex-wrap gap-2">
                    ...
                    <div className="flex flex-row gap-1 flex-wrap">
                        ...
                        <div className="flex gap-1 flex-wrap">
                            {pageNumbers.map((p, i) =>
                                p === '…' ? (
                                    <span key={`ellipsis-${i}`} className="px-3 py-2 text-sm text-gray-500 self-center">…</span>
                                ) : (
                                    <button
                                        key={p}
