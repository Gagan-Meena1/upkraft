// app/Api/relationship-manager/sessions/route.ts
...
export async function GET(request: NextRequest) {
    try {
        ...
        const rmId = decodedToken && typeof decodedToken === 'object' && 'id' in decodedToken
            ? decodedToken.id
            : null;
        ...
        const assignedTutors = await User.find({
            category: "Tutor",
            relationshipManager: rmId,
        }).select("_id");

        const instructorIds = assignedTutors.map(tutor => tutor._id);
