...
// PUT — update an existing demo slot (time, societies)
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const { tutorId, slotId, startTime, endTime, societyIds } = body;
    ...
    const slot = (tutor.demoSlotsAvailable || []).find((s: any) => s._id?.toString() === slotId);
    ...
    if (societyIds && Array.isArray(societyIds)) slot.societyIds = societyIds;

    await tutor.save();

    return NextResponse.json({ success: true, message: "Slot updated successfully" }, { status: 200 });
  } catch (error) {
