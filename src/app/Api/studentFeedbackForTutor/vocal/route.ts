...
export async function POST(request: NextRequest) {
    try {
        ...
        const feedbackData = await feedbackVocal.find({
            classId: { $in: classIds },
            userId: studentId
        }).exec();

        // Optionally, get all feedback for these classes (not just for this student)
        const feedbackAllStudent = await feedbackVocal.find({
            classId: { $in: classIds }
        }).exec();
        ...
        console.error('[API/studentFeedbackForTutor/vocal][POST] Error:', error);
        return NextResponse.json({
            success: false,
            message: error.message || 'Failed to fetch feedback',
            error: error.stack
