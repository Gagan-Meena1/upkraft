...
export async function GET(
  ...
  try {
    ...
    // Resolve instructor ID from token
    const token = (() => {
      const referer = request.headers.get("referer") || "";
      let refererPath = "";
      try { if (referer) refererPath = new URL(referer).pathname; } catch { }
    ...
    const [courseDetails, classDetails, enrolledStudents, instructor] = await Promise.all([
      ...
      User.find({ courses: courseId, category: "Student" })
        .select("-password -forgotPasswordToken -verifyToken")
        .lean(),
      instructorId
        ? User.findById(instructorId).select("academyId").lean()
