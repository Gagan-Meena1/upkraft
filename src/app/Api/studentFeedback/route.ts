...
export async function POST(request: NextRequest) {
  try {
    ...
    if (attendanceIndex !== -1) {
      ...
      user.attendance[attendanceIndex].status = attendanceStatus === "Absent" ? "absent" : "present";
    } else {
    ...
    const feedbackData = {
      userId: studentId,
      classId: classId,
      attendance: attendanceStatus === "Absent" ? 0 : 1, // Store numerical representation of attendance
