...
export async function POST(request: NextRequest) {
  try {
    ...
    const newFeedback = await feedbackDrawing.create(feedbackPayload);
    ...
    const studentFeedbacks = await feedbackDrawing.find({ userId: studentId, classId: { $in: classIds } });
    ...
    return NextResponse.json({ success: true, message: "Drawing feedback submitted", data: { feedback: newFeedback, updatedClass } }, { status: 201 });
  } catch (error: any) {
    console.error("Error submitting Drawing feedback:", error);
    return NextResponse.json({ success: false, message: error.message || "Failed to submit Drawing feedback" }, { status: 500 });
  }
}
