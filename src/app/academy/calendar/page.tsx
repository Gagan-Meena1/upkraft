...
import { setCoursesData } from "@/store/slices/userSlice";
...
const AcademyCalendarView = () => {
  ...
  const [subCategoryFilter, setSubCategoryFilter] = useState<string>("All SubCategories");
  const dispatch = useDispatch();

  const userData = useSelector((state: RootState) => state.user.userData);
  const courseData = useSelector(
    (state: RootState) => state.user.courseData
  );

  useEffect(() => {
    if (userData) {
      setUserTz(userData?.timezone);
    } else {
    ...
    const fetchCourses = async () => {
      try {
        ...
        if (Array.isArray(data.course)) {
          ...
          dispatch(setCoursesData({ courseData: data.course }));
        }
      } catch (e) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (categoryFilter !== "All Categories") {
        classParams.append("category", categoryFilter);
      }
      if (subCategoryFilter !== "All SubCategories") {
        classParams.append("subCategory", subCategoryFilter);
      }
      if (courseFilter !== "All Courses") {
        classParams.append("courseId", courseFilter);
      }

      const res = await fetch(`/Api/classes?${classParams.toString()}`);
      const data = await res.json();
  ...
  }, [currentDate, categoryFilter, subCategoryFilter, courseFilter]);
  ...
  const isSameDayInTz = useCallback((date1: Date | string, date2: Date, tz: string) => {
  ...
  // Get classes for a specific date - now simpler since filtering is done on server
  ...
  }, [allClasses, courses, isSameDayInTz, userTz]);
                ...
                {generateMonthDays(currentDate).map((day, idx) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`min-h-[140px] p-2 border rounded-lg hover:shadow-md transition-all cursor-pointer ${isToday
                    ...
                    >
                      {/* Date */}
                      <div
                        className={`text-sm font-semibold mb-2 ${isToday ? "text-purple-700" : "text-gray-900"
                          }`}
                      >
                        {day.getDate()}
                      </div>
                                ...
                                >
                                  <div className="relative bg-gray-900 text-white rounded-lg shadow-xl p-4 min-w-[250px] max-w-[400px] w-max">
                                    ...
                                    <div className="text-gray-300 text-sm mb-2 flex items-center gap-2">
                                      ...
                                      <span className={`font-medium ${classItem.studentEnrolledCount >= classItem.maxStudentCount
                                        ? 'text-red-400'
...
//     //   try {
//     //     const response = await fetch("/Api/users/usertimezone");
...
//                       key={idx}
//                       href={`/academy/calendar/day?date=${day.toISOString()}`}
//                       className={`min-h-[140px] p-2 border rounded-lg hover:shadow-md transition-all cursor-pointer overflow- ${isToday
//                         ? "bg-purple-50 border-purple-300"
//                         : "bg-white border-gray-200 hover:border-purple-200"
//                         }`}
//                     >
