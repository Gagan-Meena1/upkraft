...
const AssignmentCompletion: React.FC<AssignmentCompletionProps> = ({
  feedbackData,
  averageSkillScores,
  allStudentsFeedbackData = [],
}) => {
  const labels = feedbackData.map((item, idx) => item.sessionNo ?? idx + 1);
  const yourScores = feedbackData.map((item) => item.assignment ?? 0);
  const topScores =
    allStudentsFeedbackData.length > 0
      ? allStudentsFeedbackData.map((item) => item.assignment ?? 0)
      : [];
    ...
    datasets: [
      {
        label: "Top Score",
        data:
          topScores.length === labels.length
            ? topScores
  ...
  return (
    <div style={{ width: "100%", margin: "auto" }}>
      <h3 style={{ textAlign: "center" }}>Assignment Completion</h3>
