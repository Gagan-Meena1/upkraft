...
const RhythmChart: React.FC<RhythmChartProps> = ({
  feedbackData,
  averageSkillScores,
  ...
  const yourScores = feedbackData.map(item => item.rhythm ?? 0);
  const topScores =
    allStudentsFeedbackData.length > 0
      ? allStudentsFeedbackData.map(item => item.rhythm ?? 0)
      : [];
  ...
  return (
    <div style={{ width: "100%", margin: "auto" }}>
      <h3 style={{ textAlign: "center" }}>Rhythm</h3>
