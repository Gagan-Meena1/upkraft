...
interface MyCourseProps {
  ...
  setStartedFromDate: (date: string) => void;
}
...
const MyCourse = ({
        ...
        const entries = await Promise.all(
          Array.from(ids).map(async (id) => {
            try {
              ...
              const user = Array.isArray(data?.tutors) ? data.tutors[0] : null;
  ...
  // Reset to page 1 when search query or date filter changes
  useEffect(() => {
    if (searchQuery || startedFromDate) {
      setCurrentPage(1);
    }
  }, [searchQuery, startedFromDate, setCurrentPage]);
      ...
      <div className="assignments-list-sec">
          ...
          <div className="right-form">
            <Form>
              <div className="right-head d-flex align-items-center gap-2 flex-md-nowrap flex-wrap">
                ...
                {/* ── Started From Date Filter ── */}
                <div className="position-relative d-flex align-items-center">
                  <Form.Group className="position-relative mb-0 d-flex align-items-center gap-2">
                    Started From
                    <Form.Control
                      type="date"
                      value={startedFromDate}
                      onChange={(e) => setStartedFromDate(e.target.value)}
                      title="Filter by Started From date"
                      style={{ minWidth: 150, paddingRight: startedFromDate ? "2rem" : undefined }}
                    />
                    {/* Clear button — shown only when a date is selected */}
                    {startedFromDate && (
                      <button
                        type="button"
                        onClick={handleClearDate}
                        title="Clear date filter"
                        className="border-0 bg-transparent p-0 position-absolute d-flex align-items-center justify-content-center"
                        style={{ right: 6, top: "50%", transform: "translateY(-50%)", zIndex: 2, cursor: "pointer" }}
                      >
                ...
                {!academyId && category !== "Academic" && (
                  ...
                  >
                    ...
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M11.9997 8.66536H8.66634V11.9987C8.66634 12.1755 8.5961 12.3451 8.47108 12.4701C8.34605 12.5951 8.17649 12.6654 7.99967 12.6654C7.82286 12.6654 7.65329 12.5951 7.52827 12.4701C7.40325 12.3451 7.33301 12.1755 7.33301 11.9987V8.66536H3.99967C3.82286 8.66536 3.65329 8.59513 3.52827 8.4701C3.40325 8.34508 3.33301 8.17551 3.33301 7.9987C3.33301 7.82189 3.40325 7.65232 3.52827 7.52729C3.65329 7.40227 3.82286 7.33203 3.99967 7.33203H7.33301V3.9987C7.33301 3.82189 7.40325 3.65232 7.52827 3.52729C7.65329 3.40227 7.82286 3.33203 7.99967 3.33203C8.17649 3.33203 8.34605 3.40227 8.47108 3.52729C8.5961 3.65232 8.66634 3.82189 8.66634 3.9987V7.33203H11.9997C12.1765 7.33203 12.3461 7.40227 12.4711 7.52729C12.5961 7.65232 12.6663 7.82189 12.6663 7.9987C12.6663 8.17551 12.5961 8.34508 12.4711 8.4701C12.3461 8.59513 12.1765 8.66536 11.9997 8.66536Z" fill="white" />
                    </svg>
                  </Link>
                )}
                {category === "Academic" && (
        ...
        {startedFromDate && (
          <div className="d-flex align-items-center gap-2 mb-3">
            <span className="badge d-flex align-items-center gap-1 px-3 py-2"
              style={{ backgroundColor: "#f3e8ff", color: "#6E09BD", fontSize: 13, fontWeight: 500, borderRadius: 20 }}
            >
              <CalendarDays size={13} />
              Showing courses from{" "}
              <strong>
                {new Date(startedFromDate + "T00:00:00").toLocaleDateString("en-IN", {
              ...
              <button
                type="button"
                onClick={handleClearDate}
                className="border-0 bg-transparent p-0 ms-1 d-flex align-items-center"
                style={{ cursor: "pointer", lineHeight: 1 }}
              >
                <X size={13} style={{ color: "#6E09BD" }} />
              </button>
            </span>
          </div>
        )}
        ...
        {courses.length === 0 ? (
          <div className="text-center py-5">
            <CalendarDays size={40} className="text-muted mb-3" />
            <p className="text-muted mb-1">No courses found</p>
            {startedFromDate && (
              <p className="text-muted small">
                No courses started from{" "}
                {new Date(startedFromDate + "T00:00:00").toLocaleDateString("en-IN", {
                  day: "2-digit", month: "short", year: "numeric",
                })}{" "}
                onwards.{" "}
              ...
              <div className="w-100">
                <div className="d-flex align-items-center justify-content-left mb-3 flex-wrap gap-3">
                  ...
                  <div className="relative d-inline-block">
                    ...
                    {category === "Academic" && (
                      <div
                        className="d-none position-absolute bg-white border rounded shadow-sm p-2 mt-2"
                        style={{ minWidth: 220 }}
                      >
                          ...
                          {(() => {
                            ...
                            if (Array.isArray(ai) && ai.length > 0) {
                              ai.forEach((a: any) => {
                                if (!a) return;
                                const id = typeof a === "string" ? a : String(a._id || a);
                                if (!studentIdsSet.has(id) && !tutorsList.includes(id)) tutorsList.push(id);
                              });
                            }
                            const inst = (course as any).instructorId;
                            if (inst) {
                              const idStr = typeof inst === "string" ? inst : String(inst._id || inst);
                              if (!studentIdsSet.has(idStr) && !tutorsList.includes(idStr)) tutorsList.unshift(idStr);
                            }
                            if (tutorsList.length === 0) return <div className="text-sm text-muted">No tutors assigned</div>;
                            return tutorsList.slice(0, 8).map((tId) => {
                              const name = (tutorNameMap && tutorNameMap[tId]) || "Tutor";
                              return (
                                <div key={tId} className="bg-white border rounded px-2 py-1 text-sm text-gray-700">
                                  {name}
                                </div>
                              );
                            });
