...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const loadPendingFeedback = async () => {
      try {
        // Use the same API as the feedback-pending page so count matches
        ...
        const classesCount = Array.isArray(feedbackData?.missingFeedbackClasses)
          ? feedbackData.missingFeedbackClasses.length
          : (feedbackData.count ?? 0);
        setPendingFeedbackCount(classesCount);
        console.log("Pending feedback classes count:", classesCount);
       } catch (error) {
