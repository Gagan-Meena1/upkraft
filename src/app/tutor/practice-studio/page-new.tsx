...
export default function PracticeStudio() {
    ...
    try {
      ...
      const at = new windowWithAlphaTab.alphaTab.AlphaTabApi(alphaTabRef.current, {
        ...
        display: {
          ...
          resources: {
            // Custom styling for the notation - dark blue theme
      ...
      at.playerPositionChanged.on((e: any) => {
        ...
        setCurrentSong(prev => ({
          ...prev,
          currentTime: fmt(e.currentTime),
          totalTime: fmt(e.endTime)
        }));
      });
  ...
  const handleTrackRenderToggle = (trackIndex: number) => {
    ...
    const currentTracks = Array.from(document.querySelectorAll('.render:checked'))
      .map(el => parseInt((el.closest('.track') as HTMLElement)?.getAttribute('data-track-index') || '0'))
      .filter(idx => !isNaN(idx))
      .map(idx => alphaTabApi.score.tracks[idx])
      .filter((t: any) => !!t);
    
    // Check if the current track is already rendered
    const isCurrentlyRendered = currentTracks.some((t: any) => t.index === trackIndex);
    
    // Add or remove current track from rendered tracks
    const tracksToRender = isCurrentlyRendered
      ? currentTracks.filter((t: any) => t.index !== trackIndex)
      : [...currentTracks, alphaTabApi.score.tracks[trackIndex]];
    ...
    <>
      ...
      <div className="flex flex-col h-full bg-[#0b1220]">
        ...
        <div className="bg-[#0f1a2f] border-b border-[#1d2946] p-2 flex items-center flex-wrap gap-2 overflow-x-auto sticky top-0 z-10">
          ...
          <div id="ab" className="bg-[#0f203b] border border-[#1d2946] text-[#a6b7d1] rounded-full px-3 py-1 text-sm">
      ...
      <style jsx global>{`
        ...
        .btn.active{background:#4aa3ff;color:#08101f;box-shadow:0 0 0 2px #4aa3ff inset, 0 0 8px rgba(74,163,255,.35)}
        .solo-btn.active,
        .mute-btn.active {
          background: #7bd88f;
          color: #08101f;
