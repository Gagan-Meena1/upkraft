...
import { 
  Calendar, Music, Guitar, Piano, TrendingUp, Clock, 
  ChevronDown, ChevronUp, Play, Pause, Download 
} from "lucide-react";
...
export default function PracticeHistoryPage() {
  ...
  const toggleAudioPlayback = (audioUrl, resultId) => {
    if (playingAudio[resultId]) {
      ...
      audio.play().catch(() => alert('Failed to play audio file.'));
      audio.onended = () => setPlayingAudio(prev => ({ ...prev, [resultId]: null }));
  ...
  const getFeedbackColor = (feedback) => {
    switch (feedback?.toLowerCase()) {
      ...
      case 'generally reliable': return 'bg-purple-50 border-purple-200 text-purple-800';
      case 'fair': return 'bg-yellow-50 border-yellow-200 text-yellow-800';
      case 'fell short': return 'bg-orange-50 border-orange-200 text-orange-800';
      case 'below minimum': return 'bg-red-50 border-red-200 text-red-800';
      case 'no input': return 'bg-gray-50 border-gray-200 text-gray-600';
  ...
  const getFilteredResults = () => {
    switch (activeTab) {
      case 'piano': return results.piano || [];
      case 'guitar': return results.guitar || [];
      default: 
        return [...(results.piano || []), ...(results.guitar || [])].sort(
